...
class CryptActions:
    ...
    def encrypt(namespace):
        ...
        pool_id = (
            Id(namespace.uuid.hex, IdType.UUID)
        ...
        if bool(MOPool(mopool).Encrypted()):
            raise StratisCliNoPropertyChangeError("Pool is already encrypted")
    ...
    def unencrypt(namespace):
        ...
        if not bool(MOPool(mopool).Encrypted()):
            raise StratisCliNoPropertyChangeError(
                "Pool is not at present encrypted, so it can not be unencrypted"
            )
