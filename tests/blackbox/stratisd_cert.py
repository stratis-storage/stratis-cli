...
def make_test_pool(pool_name, pool_disks):
    ...
    if return_code != 0:
        raise RuntimeError(
            "Expected return code of 0; actual return_code: %s, error_msg: %s"
            % (return_code, msg)
        )
    if not obj_path_exists:
        raise RuntimeError("obj_path_exists: %s" % obj_path_exists)
    return obj_path
...
def make_test_filesystem(pool_path, fs_name):
    ...
    if not filesystems_created:
        raise RuntimeError("filesystems created: %s" % filesystems_created)
